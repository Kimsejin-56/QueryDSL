...
public class QuerydslBasicTest {
    ...
    @Test
    public void searchAndParam() { //이 방식 선호
        Member findMember = queryFactory
                .selectFrom(member)
                .where(member.username.eq("member1"),
                        (member.age.eq(10)))
...
//                .fetchOne();
    ...
    public void concat() {
        String result = queryFactory
                .select(member.username.concat("_").concat(member.age.stringValue()))// Enum 처리 할때 주로 사용
                .from(member)
                .where(member.username.eq("member1"))
                .fetchOne();
    ...
    /**
     * 튜플은 Ropository 계층에서만 사용 (Controller, Service 계층 사용X)
     * 서비스나 컨트롤러에서는 DTO로 변환해서 사용
     */
    @Test
    ...
    @Test
    public void findDtoJPQL() { //생성자 방식만 지원
...
//                .where(allEq(usernameCond, ageCond))
    ...
    public void bulkUpdate() {
        ...
        em.flush();
        em.clear(); //영속성 컨택스트랑 DB 일관성 맞추기 위해 사용
